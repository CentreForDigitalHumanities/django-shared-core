...
def _parse_resolver(resolver, current_namespace=None, url_prefix=None, include_all=False):
    if current_namespace and resolver.namespace:
        ns = '{}:{}'.format(current_namespace, resolver.namespace)
    ...
    # Include all if parameter is true, the namespace has been added to URLS, or URLS is empty
    include_all = include_all or (ns in settings.URLS) or len(settings.URLS) == 0
